...
# 简单匹配筛选，单字段
ios_devs = nr.filter(platform='ios')
print('single field,', ios_devs.inventory.hosts)
#
# 简单匹配筛选，多字段
ios_devs = nr.filter(platform='ios', port=22)
print('multiple fileds,', ios_devs.inventory.hosts)
...
ios_devs = nr.filter(platform='ios', port=22)
chain1_devs = ios_devs.filter(city='beijing')
print('chain1 devs: ', chain1_devs.inventory.hosts)
# 
# 
chain2_devs = ios_devs.filter(platform='ios').filter(city='beijing')
...
# # netdevops_devs = nr.filter(groups__contains='netdevops')
# # print(netdevops_devs.inventory.hosts, type(netdevops_devs))

# # 字段模糊匹配
# from nornir.core.filter import F
# leaves = nr.filter(F(name__contains='leaf',platform='ios'))
